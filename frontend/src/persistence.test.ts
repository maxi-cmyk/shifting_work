...
describe('local persistence', () => {
  ...
  it('filters invalid task records', () => {
    const storage = {
      getItem: vi.fn(() => JSON.stringify({
        tasks: [
          { id: 'bad', title: 'Impossible gear', gear: 9, targetMinutes: 10, status: 'queued', position: 0, createdAt: 'now' },
          { id: 'good', title: 'Valid task', gear: 2, targetMinutes: 20, status: 'queued', position: 1, createdAt: 'now' },
        ],
      })),
    }
  ...
  it('restores an active session safely paused in neutral', () => {
    const persisted: AppState = {
      ...initialState,
      activeSession: {
        taskId: 'task-1', accumulatedSeconds: 30, lastStartedAt: new Date(Date.now() - 5_000).toISOString(), isRunning: true, currentGear: 3,
      },
    }
