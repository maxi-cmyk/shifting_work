...
describe('MVP task and gearbox flow', () => {
  ...
  it('requires the interactive first shift before entering the cockpit', async () => {
    ...
    await user.click(within(screen.getByRole('dialog')).getByRole('button', { name: /engage gear 1/i }))
  ...
  it('requires three tasks, advances their gears, and enters fullscreen focus mode', async () => {
    ...
    window.localStorage.setItem('shiftwork.mvp.state.v1', JSON.stringify({
      tasks: [], history: [], activeSession: null,
    ...
    expect(within(screen.getByRole('status')).getByText(/1 of 3 tasks loaded/i)).toBeInTheDocument()
    ...
    await user.click(screen.getByRole('button', { name: /add to queue/i }))
    expect(within(screen.getByRole('status')).getByText(/2 of 3 tasks loaded/i)).toBeInTheDocument()

    await user.type(screen.getByLabelText('Task'), 'Build interaction')
    await user.click(screen.getByRole('button', { name: /add to queue/i }))
    expect(within(screen.getByRole('status')).getByText(/3 of 3 tasks loaded/i)).toBeInTheDocument()
  ...
  it('exits focus mode and returns the active task to the queue', async () => {
    ...
    window.localStorage.setItem('shiftwork.mvp.state.v1', JSON.stringify({
      tasks: [
        { id: 'one', title: 'Active task', gear: 1, targetMinutes: 10, status: 'active', position: 0, createdAt: new Date().toISOString() },
        { id: 'two', title: 'Second task', gear: 2, targetMinutes: 20, status: 'queued', position: 1, createdAt: new Date().toISOString() },
        { id: 'three', title: 'Third task', gear: 3, targetMinutes: 30, status: 'queued', position: 2, createdAt: new Date().toISOString() },
      ],
      history: [],
      activeSession: { taskId: 'one', accumulatedSeconds: 0, lastStartedAt: new Date().toISOString(), isRunning: true, currentGear: 1 },
      preferences: { onboardingComplete: true, soundEnabled: false, reducedMotion: false },
    }))
