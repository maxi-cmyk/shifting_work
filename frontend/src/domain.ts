...
export const gearDetails: Record<DriveGear, { label: string; targetMinutes: number; intent: string }> = {
...
export function elapsedSeconds(session: ActiveSession, now = Date.now()): number {
  ...
  const liveSeconds = Math.max(0, Math.floor((now - new Date(session.lastStartedAt).getTime()) / 1000))
...
export function getRecommendation(elapsed: number, target: number, finalGear: Gear): Recommendation {
