...
export function Gearbox({
  ...
  const handlePointerDown = (event: PointerEvent<HTMLDivElement>) => {
    ...
    railRef.current = Math.abs(visualPointRef.current.y - 50) < 1
      ? { type: 'horizontal' }
  ...
  return (
    <section className={`gearbox ${compact ? 'gearbox--compact' : ''} ${immersive ? 'gearbox--immersive' : ''}`} aria-label={label}>
      {!immersive && (
      ...
      >
        ...
        <svg className="shift-gate__path" viewBox="0 0 100 100" aria-hidden="true">
          <path className="shift-gate__shadow" d="M20 18 V50 H80 V18 M50 18 V82 M20 50 V82 M80 50 V82" />
          <path className="shift-gate__channel" d="M20 18 V50 H80 V18 M50 18 V82 M20 50 V82 M80 50 V82" />
          <path className="shift-gate__edge" d="M20 18 V50 H80 V18 M50 18 V82 M20 50 V82 M80 50 V82" />
        </svg>
        <span className="shift-boot" aria-hidden="true"><i /><i /><i /></span>

        <div className="shift-lever" ref={leverRef} aria-hidden="true">
          <span className="shift-lever__stem" />
          <span className="shift-lever__knob">
            <span className="shift-lever__cap"><i /></span>
          </span>
        </div>
      ...
      {immersive && <span className="sr-only" aria-live="polite">{gear === 0 ? 'Neutral' : `Gear ${gear}`}</span>}
    </section>
  )
}
